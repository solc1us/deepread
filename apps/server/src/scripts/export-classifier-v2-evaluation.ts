...
const outputPath = resolve(
	...
	"../../../..",
	"tmp",
	"classifier-v2.1.1-diagnostics.csv",
);
...
console.log(
	"DEV ONLY: Exporting read-only classifier v2.1.1 diagnostics. No database writes are performed.",
);
...
try {
	...
	try {
		...
		console.log(
			`v2.1.1 difficulty distribution: ${formatDistribution(v2Distribution)}`,
		);
