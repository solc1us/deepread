...
The API entrypoint is `apps/server/app.ts`. It only exports the Express app. Vercel's Express framework support routes the deployment to that one app without custom rewrites; standalone `apps/server/src/index.ts` remains responsible for local `listen`, signals, and Prisma shutdown. The function uses the Node.js 22 runtime and does not depend on Bun APIs.
