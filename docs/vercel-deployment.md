...
The API entrypoint is `apps/server/app.ts`. The API build creates `dist/vercel-app.mjs`, a Node.js bundle containing the complete internal workspace runtime graph, then verifies its import and `/health` route under Node. The entrypoint loads that bundle without calling `listen`. Vercel's Express framework support routes the deployment to that one app without custom rewrites; standalone `apps/server/src/index.ts` remains responsible for local `listen`, signals, and Prisma shutdown.
